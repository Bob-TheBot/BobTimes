...
import { Trophy, Target, Users, Calendar, Loader2, X } from 'lucide-react';
...
const SportsPage: React.FC = () => {
  ...
  const sportsArticles = getStoriesBySection('sports');

  const sportsTopics = [
    { name: 'Championships', icon: Trophy },
    { name: 'Olympic Games', icon: Target },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading sports articles...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen">
        {/* Hero Section */}
        <section className="bg-gradient-to-br from-green-50 via-background to-green-50/30 py-16 md:py-24 dark:from-green-950/20 dark:to-green-950/10">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-4xl mx-auto">
              <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-green-600 to-green-500 bg-clip-text text-transparent">
                Sports Coverage
              </h1>
              <p className="text-xl md:text-2xl text-muted-foreground mb-8">
                Comprehensive reporting on professional athletics, championship competitions, and sporting excellence.
              </p>
              
              {/* Topic badges */}
              <div className="flex flex-wrap justify-center gap-3">
                {sportsTopics.map(({ name, icon: Icon }) => (
                  <Badge
                    key={name}
                    variant="secondary"
        ...
        <section className="py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8">
              <h2 className="text-2xl font-bold">Sports Stories ({sportsArticles.length})</h2>
              <p className="text-muted-foreground mt-2">
                {sportsArticles.length > 0 ? 'Latest updates from the world of sports' : 'Check back later for sports coverage'}
              </p>
            </div>
            ...
            {sportsArticles.length === 0 ? (
              <div className="text-center py-12 bg-muted/30 rounded-lg">
                ...
                <p className="text-lg text-muted-foreground mb-2">No sports articles available at the moment</p>
                <p className="text-sm text-muted-foreground">Our sports coverage will be updated soon</p>
              </div>
            ) : (
