"""Researcher agent task execution logic."""
...
from agents.researcher_agent.researcher_prompt import ResearcherPromptBuilder
from agents.researcher_agent.researcher_state import ResearcherState, ResearcherStateManager, ResearcherToolResult, TaskPhase
...
class JournalistTaskExecutor:
    """Handles the execution logic for researcher tasks."""

    def __init__(
        self,
        agent: BaseAgent,
        tool_registry: ResearcherToolRegistry,
        prompt_builder: ResearcherPromptBuilder,
        researcher_id: str
    ):
        ...
        Args:
            ...
            tool_registry: Registry of researcher tools
    ...
    async def execute_task(
        ...
        """Execute a researcher task with proper state management.
        ...
        if task.name == TaskType.WRITE_STORY and task.topic:
            logger.info(
                f"� [REPORTER-{self.researcher_id}] Starting WRITE_STORY task for topic: {task.topic}",
                topic=task.topic,
                note="Agent will choose whether to use fetch_from_memory tool or research fresh content"
            )

        logger.info(
            f"📰 [REPORTER-{self.researcher_id}] Starting task: {task.name.value}",
            agent_type="REPORTER",
        ...
        while state.iteration < state.max_iterations:
            ResearcherStateManager.advance_iteration(state)
            ...
            try:
                ...
                ResearcherStateManager.add_error(state, error_msg)
    ...
    def _check_research_limits(self, state: ResearcherState, task: JournalistTask) -> bool:
        ...
        if (state.research_iteration_count >= 4 and
            ...
            if len(state.sources) == 0:
                logger.error(
                    f"❌ [REPORTER-{self.researcher_id}] Research failed - no usable sources found",
                    agent_type="REPORTER",
                    task_type=task.name.value,
                    field=task.field.value,
                    researcher_id=self.researcher_id,
    ...
    async def _handle_tool_execution(self, tool_call: StoryToolCall, state: ResearcherState) -> None:
        ...
        ResearcherStateManager.add_tool_call(state, tool_call)
        ResearcherStateManager.add_command(
        ...
        # Create tool result
        tool_result = ResearcherToolResult(
        ...
        # Add result to state
        ResearcherStateManager.add_tool_result(state, tool_result)
        ...
        if tool_result.error:
            ResearcherStateManager.add_error(state, f"Tool {tool_call.name} failed: {tool_result.error}")

    async def _handle_unified_tool_result(self, result: Any, state: ResearcherState) -> None:
        ...
        logger.info(
            f"🔧 [REPORTER-{self.researcher_id}] Processing unified tool result",
            operation=operation,
        ...
        if hasattr(result, "sources") and result.sources:
            ...
            if operation == "search" and topic_mapping:
                ...
                logger.info(
                    f"🔗 [REPORTER-{self.researcher_id}] Stored {len(enhanced_sources)} enhanced sources from {operation} tool in SharedMemoryStore",
                ...
                logger.info(
                    f"🔗 [REPORTER-{self.researcher_id}] Stored {len(result.sources)} sources from {operation} tool in SharedMemoryStore",
                    topics_linked=len(topic_mapping) if topic_mapping else 0
                )
        else:
            logger.warning(
                f"⚠️ [REPORTER-{self.researcher_id}] Tool result has no sources or sources are empty",
                operation=operation
            )
        ...
        if operation == "youtube" and hasattr(result, "metadata") and result.metadata:
            ...
            if video_metadata:
                ...
                logger.info(
                    f"📹 [REPORTER-{self.researcher_id}] Stored {len(video_metadata)} video metadata entries for transcript retrieval",
                    total_metadata_entries=len(state.youtube_video_metadata)
                )

    async def _enhance_search_content(self, sources: list[StorySource], topic_mapping: dict[str, Any], state: ResearcherState) -> tuple[list[StorySource], dict[str, Any]]:
        ...
        for source in sources:
            ...
            if topic_info and topic_info.get('needs_content_fetch', False) and scraper_tool:
                logger.info(
                    f"🔍 [REPORTER-{self.researcher_id}] Enhancing content for topic with scraping",
                    url=source.url,
                ...
                try:
                    ...
                    from agents.researcher_agent.researcher_tools import ScrapeParams
                    ...
                    if scrape_result.success and scrape_result.sources:
                        ...
                        if scraped_source.content and len(scraped_source.content) > len(source.content or ''):
                            ...
                            logger.info(
                                f"✅ [REPORTER-{self.researcher_id}] Enhanced content via scraping",
                                url=source.url,
                            ...
                            logger.warning(
                                f"⚠️ [REPORTER-{self.researcher_id}] Scraping didn't improve content quality",
                                url=source.url
                            )
                    else:
                        logger.warning(
                            f"⚠️ [REPORTER-{self.researcher_id}] Failed to scrape content",
                            url=source.url,
                            error=scrape_result.error
                        )
                except Exception as e:
                    logger.error(
                        f"❌ [REPORTER-{self.researcher_id}] Error during content enhancement",
                        url=source.url,
                        error=str(e)
                    )
        ...
        for source in enhanced_sources:
            ...
            if content_length >= 100:  # Minimum content threshold
                ...
                logger.warning(
                    f"⚠️ [REPORTER-{self.researcher_id}] Filtering out topic with insufficient content",
                    url=source.url,
        ...
        logger.info(
            f"📊 [REPORTER-{self.researcher_id}] Content validation complete",
    ...
    def _handle_search_result(self, result: Any, state: ResearcherState) -> None:
        ...
        """
        logger.info(
            f"🔍 [REPORTER-{self.researcher_id}] Processing search result",
        ...
        if hasattr(result, "cleaned_results"):
            ...
            ResearcherStateManager.add_search_result(state, search_summary)

            # Convert to story sources and save using generic method
            story_sources = ResearcherToolRegistry.convert_search_results_to_sources(result.cleaned_results)
            ...
            logger.info(
                f"🔗 [REPORTER-{self.researcher_id}] Saved {len(story_sources)} sources from search results",
            ...
            logger.warning(
                f"⚠️ [REPORTER-{self.researcher_id}] Search result has no cleaned_results attribute"
            )

    def _handle_youtube_result(self, result: Any, state: ResearcherState) -> None:
        ...
        """
        logger.info(
            f"🎥 [REPORTER-{self.researcher_id}] Processing YouTube result",
            result_type=type(result).__name__,
            operation=getattr(result, "operation", "unknown"),
        ...
        if hasattr(result, "sources") and result.sources:
            # Use ResearcherStateManager to store sources in SharedMemoryStore
            topic_mapping = getattr(result, "topic_source_mapping", None)
            ResearcherStateManager.save_sources_with_topics(
                state=state,
                sources=result.sources,
            ...
            logger.info(
                f"🔗 [REPORTER-{self.researcher_id}] Stored {len(result.sources)} YouTube sources in SharedMemoryStore",
                topics_linked=len(topic_mapping) if topic_mapping else 0,
                operation=result.operation
            )
        else:
            logger.warning(
                f"⚠️ [REPORTER-{self.researcher_id}] YouTube result has no sources attribute or sources are empty"
            )
        ...
        if hasattr(result, "video_metadata") and result.video_metadata:
            ...
            logger.info(
                f"📹 [REPORTER-{self.researcher_id}] Stored {len(result.video_metadata)} video metadata entries for transcript retrieval",
                total_metadata_entries=len(state.youtube_video_metadata),
                operation=result.operation
            )

    def _handle_fetch_from_memory_result(self, result: Any, state: ResearcherState) -> None:
        ...
        """
        logger.info(
            f"🧠 [REPORTER-{self.researcher_id}] Processing fetch_from_memory result",
            success=getattr(result, "success", False),
        ...
        if hasattr(result, "success") and result.success:
            ...
            if topic_sources:
                ...
                logger.info(
                    f"📚 [REPORTER-{self.researcher_id}] Injected {len(topic_sources)} sources from memory into context",
                    topic_key=result.topic_key,
                    total_sources_now=len(state.sources)
                )
            else:
                logger.warning(
                    f"⚠️ [REPORTER-{self.researcher_id}] No sources found in memory for topic: {result.topic_key}"
                )
        else:
            error_msg = getattr(result, "error", "Unknown error")
            logger.warning(
                f"⚠️ [REPORTER-{self.researcher_id}] fetch_from_memory failed: {error_msg}"
            )
    ...
    def _handle_final_response(
        ...
        if task.name == TaskType.WRITE_STORY:
            ...
            if response.research_result:
                ...
                self._enhance_research_result(response.research_result, state)
                ResearcherStateManager.set_current_research(state, response.research_result)
                ResearcherStateManager.advance_research_iteration(state)

                # Check if we have sufficient research to proceed to writing
                if ResearcherStateManager.has_sufficient_research(state, task.min_sources):
                    ResearcherStateManager.transition_to_writing_phase(state)

                logger.info(
                    f"✅ [REPORTER-{self.researcher_id}] Research phase completed",
                    sources_count=len(response.research_result.sources),
            ...
            if response.story_draft:
                ...
                ResearcherStateManager.mark_completed(state)

                logger.info(
                    f"✅ [REPORTER-{self.researcher_id}] Story draft completed successfully",
    ...
    def _enhance_research_result(self, research_result: ResearchResult, state: ResearcherState) -> None:
    ...
    def _ensure_all_sources_in_story(self, story_draft: StoryDraft, state: ResearcherState) -> None:
        ...
        if sources_added > 0:
            logger.info(
                f"📚 [REPORTER-{self.researcher_id}] Added {sources_added} research sources to story draft"
            )
    ...
    def _determine_model_speed(
        ...
        if task.name == TaskType.WRITE_STORY:
            if state.task_phase == TaskPhase.WRITING:
                ...
                logger.info(f"🐌 [REPORTER-{self.researcher_id}] Using SLOW model for story writing phase")
                return ModelSpeed.SLOW
            else:
                # Use fast model for research phase (efficiency)
                logger.info(f"⚡ [REPORTER-{self.researcher_id}] Using FAST model for research phase")
